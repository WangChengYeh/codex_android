...
use crate::android_pty::AndroidPty;
use crate::exec_command::exec_command_params::ExecCommandParams;
use crate::exec_command::exec_command_params::WriteStdinParams;
use crate::exec_command::android_exec_command_session::ExecCommandSession;
...
impl AndroidSessionManager {
    ...
    pub async fn handle_exec_command_request(
        ...
        let (session, mut exit_rx) =
            create_android_exec_command_session(params.clone())
                .await
                .map_err(|err| {
                    format!(
...
/// Spawn Android PTY and child process per spawn_exec_command_session logic.
async fn create_android_exec_command_session(
    params: ExecCommandParams,
    ...
    // Create Android PTY pair
    let (master, slave) = AndroidPty::new()
        .map_err(|e| anyhow::anyhow!("Failed to create Android PTY: {}", e))?;

    // Spawn a shell into the pty
    let mut command = Command::new(shell);
    let shell_mode_opt = if login { "-lc" } else { "-c" };
    command.arg(shell_mode_opt);
    command.arg(cmd);

    let mut child = slave.spawn_command(command)?;
    
    // Get the child ID for potential killing
    ...
    // Create a simple killer function for Android
